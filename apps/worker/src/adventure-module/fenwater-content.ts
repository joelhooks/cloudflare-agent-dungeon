import { z } from "zod";
import { AdventureModuleComponentSchema, SafeHavenTemplateSchema, TreasureParcelTemplateSchema, type AdventureModuleComponent } from "@cloudflare-agent-dungeon/domain";
...
export const FENWATER_EXPANDED_COMPONENTS: AdventureModuleComponent[] = [
  ...
  component({ id: "fenwater-safehaven-alder-knoll", kind: "safeHaven", title: "Alder Knoll dry camp", visibility: "referee", payload: SafeHavenTemplateSchema.parse({ id: "fenwater-safehaven-alder-knoll", name: "Alder Knoll dry camp", description: "High dry ground where the party can get above ditch water and make short repairs under watch.", capabilities: ["rest", "stash_treasure", "recover_hp"], access: { conditions: ["someone keeps watch", "smoke may reveal the camp"], route: "fenwater-location-alder-knoll" }, services: [{ id: "dry-gear", name: "dry gear and bind wounds" }], risks: ["visible smoke", "Ditch gang routes may shift"], visibility: "referee_private" }) }),
  component({ id: "fenwater-safehaven-reedwright-stove-boat", kind: "safeHaven", title: "Reedwright stove boat", visibility: "referee", payload: SafeHavenTemplateSchema.parse({ id: "fenwater-safehaven-reedwright-stove-boat", name: "Reedwright stove boat", description: "A warm reedwright boat where fragile evidence can be protected if the family is willing.", capabilities: ["rest", "settle_treasure", "gather_rumors"], access: { conditions: ["Reedwrights are not hostile", "party accepts a guide-favor debt"], route: "fenwater-location-reedwright-camp", factionConstraints: ["reedwright trust matters"] }, services: [{ id: "route-gossip", name: "route gossip" }, { id: "warm-stove", name: "warm stove and dry wrappings" }], risks: ["Grain-buyer pressure follows favors"], visibility: "referee_private" }) }),
  component({ id: "fenwater-safehaven-pump-mezzanine", kind: "safeHaven", title: "Pump House mezzanine", visibility: "referee", payload: SafeHavenTemplateSchema.parse({ id: "fenwater-safehaven-pump-mezzanine", name: "Pump House mezzanine", description: "A narrow platform above floodwater and machinery, useful as a temporary cache more than a home base.", capabilities: ["stash_treasure"], access: { conditions: ["machinery is braced or watched"], route: "fenwater-location-old-pump-house" }, services: [{ id: "temporary-cache", name: "temporary above-water cache" }], risks: ["pump pressure advances", "machinery clues are not private if others reach the house"], visibility: "referee_private" }) }),
  component({ id: "fenwater-safehaven-reeve-hall-backroom", kind: "safeHaven", title: "Reeve Hall back room", visibility: "referee", payload: SafeHavenTemplateSchema.parse({ id: "fenwater-safehaven-reeve-hall-backroom", name: "Reeve Hall back room", description: "A locked civic back room where an old campaigner can train survivors once the party brings proof worth political shelter.", capabilities: ["rest", "train_level_up", "hire_retainers", "gather_rumors"], access: { conditions: ["Charter House proof, bailiff bonds, or a public debt-web witness gives the reeve cover to shelter the party"], route: "fenwater-location-charter-house", factionConstraints: ["reeve will not cross the bailiff without leverage"] }, services: [{ id: "old-campaigner-training", name: "old campaigner training" }, { id: "reeve-retainers", name: "retainer introductions" }], risks: ["training makes the debt-web openly political"], visibility: "referee_private" }) }),

  component({ id: "fenwater-treasure-mort-ledger-page", kind: "treasure", title: "Mort ledger page", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-mort-ledger-page", name: "Mort ledger page", description: "A water-stained ledger page that names a debt pattern and points toward the Charter House.", valuation: { kind: "requires_referee_appraisal", suggestedGpRange: { min: 0, max: 25 } }, kind: "document", encumbrance: { bulk: "negligible" }, placement: { visibility: "referee_private", locationId: "fenwater-location-morts-bar", clue: "Found if the party secures Mort's records before water or panic spoils them.", sourceNotes: "Names a debt and points to the Charter House." }, playerFacing: { description: "A damp ledger page with shell-token marks and several legible names.", visibility: "party_known" }, tags: ["evidence", "ledger", "referee:debt-web"], visibility: "referee_private" }) }),
  component({ id: "fenwater-treasure-shell-token-string", kind: "treasure", title: "Shell-token string", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-shell-token-string", name: "Shell-token string", description: "A string of marked shell tokens used to track tally traffic through the tithe mill.", valuation: { kind: "requires_referee_appraisal", suggestedGpRange: { min: 5, max: 30 } }, kind: "oddity", encumbrance: { bulk: "hand" }, placement: { visibility: "referee_private", locationId: "fenwater-location-north-ditch-door", clue: "Turns up where shell-token traffic crossed wet ground.", sourceNotes: "Proves tally traffic at the tithe mill." }, playerFacing: { description: "A cord of marked shell tokens, too deliberate to be decoration.", visibility: "party_known" }, tags: ["evidence", "shell-token", "tithe-mill"], visibility: "referee_private" }) }),
  component({ id: "fenwater-treasure-lockwheel-key", kind: "treasure", title: "Lockwheel key", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-lockwheel-key", name: "Lockwheel key", description: "A heavy maintenance key for one cistern lockwheel or valve housing.", valuation: { kind: "non_xp_leverage" }, kind: "tool", encumbrance: { bulk: "hand", notes: "heavy iron key" }, placement: { visibility: "referee_private", locationId: "fenwater-location-lockkeepers-cistern", clue: "Accessible through cistern pressure play or a recovered maintenance cache.", sourceNotes: "Opens or jams one cistern valve." }, playerFacing: { description: "A heavy iron key sized for old pump-house machinery.", visibility: "party_known" }, tags: ["tool", "cistern", "lockwheel"], visibility: "referee_private" }) }),
  component({ id: "fenwater-treasure-charter-silver-strongbox", kind: "treasure", title: "Charter House silver strongbox", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-charter-silver-strongbox", name: "Charter House silver strongbox", description: "A sealed strongbox of tithe silver and salvage writs hidden behind the Charter House mezzanine records.", valuation: { kind: "fixed_gp", valueGp: 4800 }, kind: "coin", encumbrance: { bulk: "sack", notes: "heavy enough to make retreat matter" }, placement: { visibility: "referee_private", locationId: "fenwater-location-charter-house", clue: "Found if the party secures Charter House proof and escapes with the strongbox before the bailiff claims jurisdiction.", sourceNotes: "Large enough to make a level-up possible after recovered-to-safety settlement." }, playerFacing: { description: "A sealed strongbox heavy with old tithe silver and salvage writs.", visibility: "party_known" }, tags: ["treasure", "silver", "charter", "strongbox"], visibility: "referee_private" }) }),
  component({ id: "fenwater-treasure-bailiff-bond-cache", kind: "treasure", title: "Bailiff bond cache", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-bailiff-bond-cache", name: "Bailiff bond cache", description: "A waxed packet of enforceable debt bonds and silver clips hidden where the bailiff's runners can retrieve it after the flood.", valuation: { kind: "fixed_gp", valueGp: 3600 }, kind: "document", encumbrance: { bulk: "scroll_case", notes: "fragile waxed packet; worthless if soaked or burned" }, placement: { visibility: "referee_private", locationId: "fenwater-location-charter-house", clue: "Recovered by choosing records over an easy exit while bailiff pressure is active.", sourceNotes: "A risky evidence haul that is valuable only after SafeHaven settlement." }, playerFacing: { description: "A waxed packet of bailiff bonds, silver clips, and names that could buy enemies or levels if it reaches safety.", visibility: "party_known" }, tags: ["treasure", "bonds", "bailiff", "evidence"], visibility: "referee_private" }) }),
  component({ id: "fenwater-treasure-pump-house-salvage-cache", kind: "treasure", title: "Pump House salvage cache", visibility: "referee", payload: TreasureParcelTemplateSchema.parse({ id: "fenwater-treasure-pump-house-salvage-cache", name: "Pump House salvage cache", description: "A lashed cache of silver-inlaid valve parts, tool rolls, and paid salvage claims under the old pump house catwalk.", valuation: { kind: "fixed_gp", valueGp: 2800 }, kind: "trade_good", encumbrance: { bulk: "sack", notes: "awkward metal salvage; slows retreat through water" }, placement: { visibility: "referee_private", locationId: "fenwater-location-old-pump-house", clue: "Found if the party investigates the pump house machinery and chooses haul over speed.", sourceNotes: "Treasure with a real extraction problem." }, playerFacing: { description: "A heavy salvage cache of silver-inlaid pump parts and paid claim tags.", visibility: "party_known" }, tags: ["treasure", "salvage", "pump-house", "silver"], visibility: "referee_private" }) }),
